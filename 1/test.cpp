...
SUITE(KeyTest){
    ...
    TEST(test1) {
        CHECK(modAlphaCipher(L"ИБСТ").encrypt(L"ЖЕСТ")==L"ПЁГЕ");
    }
    TEST(test2) {
        CHECK(modAlphaCipher(L"ГРАНТАГРАНТА").encrypt(L"ЛАДА")==L"ОРДН");
    }
    TEST(test3) {
        CHECK(modAlphaCipher(L"ибст").encrypt(L"ЖЕСТ")==L"ПЁГЕ");
    }
    TEST(test4) {
        CHECK_THROW(modAlphaCipher(L"СВЕТА1"), cipher_error);
    }
    TEST(test5) {
        CHECK_THROW(modAlphaCipher(L"СВ,ЕТА"), cipher_error);
    }
    TEST(test6) {
        CHECK_THROW(modAlphaCipher(L"СВ ЕТА"), cipher_error);
    }
...
SUITE(DecryptTest)
    ...
    TEST_FIXTURE(KeyB_fixture, Punct) {
        CHECK_THROW(p->decrypt(L"СВЕТА+ТЁМА"), cipher_error);
        }
    TEST_FIXTURE(KeyB_fixture, Empty) {
